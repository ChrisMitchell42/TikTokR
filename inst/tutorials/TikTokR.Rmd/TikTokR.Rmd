...
devtools::install_github("ChrisMitchell42/TikTokR")

tiktok <- TikTokR::tutorialdata
tiktok[,c(2:5)] <- (tiktok[,c(2:5)])
```
...
Let's have a look at this Tiktok data below. Firstly, just have a glance at the different headings there are and the sort of values that are displayed. The Id variable is just how the Tiktok users are identified and next to them is their share counts, playcounts, comment counts and digg counts. 
...
Don't panic! Let's break this down quickly and explain why this looks like this:

`1.` Subset is the function which is telling the computer to, unsurprisingly, subset the tiktok data for those users that have a share count over 50.
...
The final task is to plot a scatterplot of the playCount against the shareCount. To do this we use the function plot(). Remember that you must tell the computer what dataset you are getting your variables from and these must be separated by a comma!

```{r plot, exercise=TRUE}

```
```{r plot-solution}
plot(tiktok$playCount,tiktok$shareCount)
```
